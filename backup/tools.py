...
def ping_check(host):
    ...
    if output: #BAD BAD BAD, REWORK ALL BELOW!!
        latency = 0
        result = output.split('\n')
        for i in result:
...
def src_check(src): 
    try:
        if os.path.ismount(src): #If it's mounted.
            ...
            try:
                ...
                sleep(2)
                if os.path.ismount(src):#Check if mounted after repair attempt.
                ...
                print("[!] Failed to run (mount -a). Exiting script!" % src)
                #EMAIL ALERT
                sys.exit()
    except:
    ...
    for dirName, subdirList, fileList in os.walk(src):
        for fname in fileList:
            ...
            for ext in crypto_ext:
                if re.search(ext, fname.lower()): #TODO: MOVE THIS SHIT OUT OF IF STATEMENT!
                    infected += 1
